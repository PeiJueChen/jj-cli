...
const updateAppDesc = async (currentWorkingDir, appName, env, appInfo) => {
    const path = require('path');
    const fs = require('fs').promises;
    
    let currentPath = currentWorkingDir;
    const version = appInfo?.buildVersion;
    const buildUpdateDescription = appInfo?.buildUpdateDescription || "";
    const buildKey = appInfo?.buildKey || "";
    const {apiKey, userKey} = appInfo;
    if (!version || !apiKey || !userKey || !buildKey) return;
    
    while (currentPath !== path.parse(currentPath).root) {
        const targetFile = path.join(currentPath, '.app-versions.json');
        
        try {
            await fs.access(targetFile); // 检查文件是否存在
            const content = await fs.readFile(targetFile, 'utf8');
            const apps = content && JSON.parse(content) || {};
            const app = apps[appName];
            const envData = app?.[env] || [];
            const versionData = envData.find(item => item.version === version);
            const appByodVersion = versionData?.appByodVersion;
            const branch = versionData?.branch;
            let result = buildUpdateDescription;
            if (appByodVersion) result += `\n* appByodVersion: ${appByodVersion}`;
            if (branch) result += `\n* source: ${branch}`;
            updateAppInfo(apiKey, userKey, buildKey, result);
            return;
        } catch (error) {
            // 文件不存在则继续向上查找
...
const uploadToPgyer = async (app, platform, apiKey, appPath, buildChannelShortcut, buildUpdateDescription, env, currentWorkingDir, userKey) => {
    ...
    try {
        ...
        const appInfo = rsp?.data || {};
        appInfo['apiKey'] = apiKey;
        appInfo['userKey'] = userKey;
        updateAppDesc(currentWorkingDir, app, env, appInfo);
