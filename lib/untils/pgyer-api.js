...
const uploadToPgyer = async (apiKey, appPath, buildChannelShortcut, buildUpdateDescription, env) => {
...
const getAllVerions = async (apiKey, appKey, page) => {
    ...
    const currentPage = rsp?.data?.currentPage;
...
const getNextVerion = async (apiKey, appKey, env) => {
    ...
    const rs = list.filter(version => {
        return version?.buildUpdateDescription?.includes(getEnvString(env))
    })
