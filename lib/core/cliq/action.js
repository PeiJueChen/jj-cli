...
let clientID_;
let secret_;
let scope = 'ZohoCliq.Channels.CREATE,ZohoCliq.Channels.READ,ZohoCliq.Channels.UPDATE,ZohoCliq.Channels.DELETE,ZohoCliq.Bots.CREATE,ZohoCliq.Bots.READ,ZohoCliq.Bots.UPDATE,ZohoCliq.Bots.DELETE,ZohoCliq.Chats.CREATE,ZohoCliq.Chats.READ,ZohoCliq.Chats.UPDATE,ZohoCliq.Chats.DELETE,ZohoCliq.Buddies.CREATE,ZohoCliq.Buddies.READ,ZohoCliq.Buddies.UPDATE,ZohoCliq.Buddies.DELETE,ZohoCliq.Messages.READ,ZohoCliq.Attachments.READ,ZohoCliq.Webhooks.CREATE,ZohoCliq.StorageData.ALL';
...
const cliq = async ({ currentWorkingDir, cliBinDir, email, token }) => {
    ...
    if (!config_ && defaultRefreshToken) {
        await refreshToken(defaultRefreshToken);
    }

    if (config_) {
        config_['refresh_token'] = config_['refresh_token'] || defaultRefreshToken;
    }
...
const sendFile = async (file) => {
    ...
    try {
        ...
        if (r.code.startsWith(2) || r.code.startsWith('2')) {
...
const getAccessToken = async (code) => {
    ...
    try {
        ...
        if (rs.code.startsWith(2) || rs.code.startsWith('2')) {
