...
const gpushAction = async (message, branch, depository) => {
    ...
    try {
        await common.runTask("add...", async () => {
            await runCommand(`git add .`);
        })
        await common.runTask("commit...", async () => {
            await runCommand(`git commit -m "${message}"`);
        })
        await common.runTask("push...", async () => {
            await runCommand(`git push ${depository} ${branch}`);
        })
...
const gtagAction = async (tagName, depository) => {
    ...
    try {
        await common.runTask("tag...", async () => {
            await runCommand(`git tag ${tagName}`);
        })
        await common.runTask("push...", async () => {
            await runCommand(`git push ${depository} ${tagName}`);
        })
    } catch (error) {
        logs.logError(error);
    }
...
const getDate = async () => {
    var date = await runCommand("date +'%Y-%m-%d-%H%M%S'");
    return date.replace(/\n/g, "");
}
