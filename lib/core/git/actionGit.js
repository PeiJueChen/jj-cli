...
const gClearAction = async (clear, depository) => {
    ...
    try {
        await common.runCommandWithTask("Clear remote depository cache...\n", `git remote prune ${depository}`);
    } catch (error) {
        logs.logWarn("Clear remote depository cache error: ", error);
    }

    try {
        await common.runCommandWithTask("Aggressive prune...\n", `git gc --aggressive --prune=now`);
    } catch (error) {
    ...
    try {
        ...
        await common.runCommandWithTask("Delete all remote tracking branches...\n", command);
    } catch (error) {
        logs.logWarn("Delete all remote tracking branches error: ", error);
    }

    try {
        await common.runCommandWithTask("Delete all local tags...\n", `git tag | xargs git tag -d`);
    } catch (error) {
