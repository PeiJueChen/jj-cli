const { runSpawnCommand, commandExec, runCommand, runCommandAndOutput } = require('../../untils/terminal');
...
const gClearAction = async (clear, depository) => {
    if (!clear || clear != 'clear') {
        logs.logFatal('you mean git clear ?');
        return;
    }
    const isInstall = await common.isInstalled('git');
    if (!isInstall) {
    ...
    await common.runTaskAndLog("Clear remote depository cache...\n", async () => {
        try {
            await runCommandAndOutput(`git remote prune ${depository}`);
        } catch (error) {
            logs.logWarn('Clear remote depository error: ', error);
        }
    })
    
    await common.runTaskAndLog("Aggressive prune...\n", async () => {
        try {
            await runCommandAndOutput(`git gc --aggressive --prune=now`);
        } catch (error) {
            logs.logWarn("Aggressive prune error: ", error);
        }
    })

    await common.runTaskAndLog("Delete all remote tracking branches...\n", async () => {
        try {
            const command = `git branch -r | grep -v "$(git rev-parse --abbrev-ref HEAD)" | sed "s#${depository}/##" | xargs -I {} git branch -dr ${depository}/{}`;
            await runCommandAndOutput(command);
        } catch (error) {
            logs.logWarn("Delete all remote error: ", error);
        }
    })

    await common.runTaskAndLog("Delete all local tags...\n", async () => {
        try {
            await runCommandAndOutput(`git tag | xargs git tag -d`);
        } catch (error) {
            logs.logWarn("Delete all local tags error: ", error);
        }
    })
