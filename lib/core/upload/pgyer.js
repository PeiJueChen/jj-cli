...
const pgyerUpload = async (platform, appPath, project, config, closedfolder) => {
    ...
    try {
        ...
        console.log("exec error result:", result);
        logs.logFatal(error);
        return;
    }
    ...
    if (typeof result === 'string') {
        try {
            ...
            console.log("parse error result:", result);
            logs.logFriendly(error);
        }
    }
    ...
    if (!closedfolder) await common.openUrl(openUrl);
    process.exit(1);
